/***********************************************************************
  ...
  ---------------
    runs/um_YYYYMMDD_HHMMSS_GPUx_<uuid>/
      run.json      -  machine-readable full result (schema 2.6)
...
int main(int argc, char** argv) {
  ...
  std::cout << "\n" << HDR << "\n";
  std::cout << "  CUDA Unified Memory Analyzer  V8.1\n";
  std::cout << HDR << "\n\n";
  ...
  std::cout << std::fixed << std::setprecision(2)
            << "  VRAM     " << vram_gib << " GiB"
            << "  " << nv.mem_type << "  " << nv.mem_bus_width << "-bit\n";
  ...
  auto live_line = [&](const char* tag, const ResultRow& row) {
    ...
    std::cout << "    "
              ...
              << std::setw(14) << row.regime
              << "p50=" << std::setprecision(2) << row.t.steady_p50_ms << "ms"
              << "  p90=" << row.t.steady_p90_ms << "ms"
              << "  p99=" << row.t.steady_p99_ms << "ms"
              << "  max=" << row.t.steady_max_ms << "ms"
              << "  tail=" << std::setprecision(2) << row.t.steady_tail_ratio << "x"
  ...
  // memory line
  {
    ...
    if (uma_platform)
      v << "alloc=" << gib(host.uma_allocatable) << " GiB"
        << "  ceil=" << std::setprecision(0) << host.ceiling_utilization * 100.0 << "%";
    else
      v << "host=" << gib(host.uma_allocatable) << " GiB"
  ...
  write_run_json(run_dir, nv, host, host_cap_gib, cold_rows, warm_rows, pressure_rows, cold_k, warm_k, pressure_enabled, cold_child_failures,
                 ...
                 um_direction_ratio, um_direction_trend);
