/***********************************************************************
  ...
  --------------
    ...
    DEGRADED        — pressure never settled, pingpong, or throttle detected
    CRITICAL        — cold child failure, invalid thermal, or memory danger

  EVIDENCE BUNDLE
  ---------------
    runs/um_YYYYMMDD_HHMMSS_GPUx_<uuid>/
      run.json     — machine-readable full result (schema 2.4)
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < runnable.size(); ++i) {
    ...
    // Step 2: CPU first-touch — commits pages at 4KB CPU granularity
    for (size_t j = 0; j < n_floats; j += stride) p[j] = 1.0f;

    // Step 3: prefetch to GPU — driver migrates to 2MB GPU pages
    ...
    gpu_touch_ms(p, n_floats, stride);
    ...
    // Step 5: free — releases GPU pages, next ratio starts clean
    cudaFree(p);
  ...
  if (PRESSURE_REPEATS >= 2) {
    ...
    if (all_low) {
      ...
      // Criterion 2: per-step delta within 20% of prior cv
      for (int i = 1; i < PRESSURE_REPEATS; ++i) {
        ...
        if (delta < threshold) { pressure_stable_pass = i + 1; break; }
      }
  ...
  write_run_json(run_dir, nv, host, host_cap_gib, cold_rows, warm_rows, pressure_rows, cold_k, warm_k, pressure_enabled, cold_child_failures,
                 ...
                 hw, transport, nvlink_c, prop, pcie_replay_delta, pcie_bw, thrash, reswin);
  ...
  // pressure line
  {
    ...
    if (pressure_stable_pass > 0) {
      ...
      if (all_low_cv_summary) line << "  stable=LOW_CV_ALL";
      else line << "  stable=pass" << pressure_stable_pass;
    } else {
      line << "  stable=no";
    }
  ...
  // Migration / thrash diagnostic — two lines for readability
  ...
  std::cout << std::left << std::setw(10) << ""
            ...
            << "  thrash_score=" << std::setprecision(2) << thrash.thrash_score
            << "  state=" << thrash.state << "\n";
